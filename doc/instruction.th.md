...
> ให้ไปที่ [ภาคผนวก A](#ภาคผนวก-a) แทน — แต่ §8 (ความต่างของเวอร์ชัน) และ §9 (แก้ปัญหา)
> ยังใช้กับคุณอยู่
...
## 8. ความต่างของเวอร์ชันที่คุณจะสังเกตเห็น

toolchain บนเครื่องคุณใหม่กว่าที่ใช้ใน container โดยตั้งใจ ซึ่งไม่เป็นไร เพราะ Go และ Node
รองรับย้อนหลังในทิศทางที่จำเป็นอยู่แล้ว แต่ควรรู้ไว้ จะได้ไม่เข้าใจผิดว่าเป็นบั๊ก

| ที่ไหน                          | เวอร์ชันตรงนั้น              | ของคุณ    |
| ------------------------------ | -------------------------- | --------- |
| `store-service/go.mod`         | ระดับภาษา `go 1.25.0`       | Go 1.26.5 |
...
Go toolchain ที่ใหม่กว่าคอมไพล์โมดูล `go 1.25` ได้ตามปกติ **อย่าไปแก้ค่า `go` ใน `go.mod`
ให้ตรงกับเวอร์ชันบนเครื่องคุณ** เพราะจะทำให้ Docker build พัง

`store-web` ใช้ Next.js 14.0.4 ซึ่งออกมาก่อน Node 24 ในทางปฏิบัติมัน build และรันได้
แต่ถ้าคุณเจอ error แปลก ๆ ตอน `next dev` หรือ `next build` ที่ไม่มีใครเจอ ให้ลองลดลงไปใช้
เวอร์ชันเดียวกับใน container เพื่อยืนยันก่อนไล่หาสาเหตุ: `nvm install 21 && nvm use 21`
